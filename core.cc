  ...
  void Simulator<dim>::run ()
  {
    ...
    if (parameters.resume_computation == true)
    ...
    else
      {
        ...
        // refine_global(n).
    for (unsigned int n=0; n<parameters.initial_regional_refinement; ++n)
          {
            for (const auto &cell : triangulation.active_cell_iterators())
              if (cell->is_locally_owned()){
                if(n<parameters.initial_global_refinement) cell->set_refine_flag ();
                else if(n<parameters.initial_subregional_refinement){
                  for(unsigned int i=0;i<cell->n_vertices();++i){
                    double x=cell->vertex(i)(0);
                    double y=cell->vertex(i)(1);
                    double z=cell->vertex(i)(2);
                    double r=sqrt(x*x+y*y+z*z);
                    double theta=acos(z/r)*180/3.14159;
                    double phi=atan2(y,x)*180/3.14159;
                    
                    if(theta>parameters.min_theta_sub&&theta<parameters.max_theta_sub){
                      if(parameters.min_phi_sub<parameters.max_phi_sub&&phi>parameters.min_phi_sub&&phi<parameters.max_phi_sub)
                  ...
                  for(unsigned int i=0;i<cell->n_vertices();++i){
                    ...
                    if(theta>parameters.min_theta&&theta<parameters.max_theta){
                      if(parameters.min_phi<parameters.max_phi&&phi>parameters.min_phi&&phi<parameters.max_phi)
                        cell->set_refine_flag ();
                      if(parameters.min_phi>parameters.max_phi&&(phi<parameters.min_phi||phi>parameters.max_phi))
                        cell->set_refine_flag ();
                    }
